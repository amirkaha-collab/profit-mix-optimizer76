...
.block-container { padding: 2.2rem 1.2rem 3rem !important; max-width: 1180px; }
...
def _chip(label, val, fmt="{:.1f}%", lower_better=False, baseline_val=None):
    ...
    if baseline_val is not None:
        try:
            ...
            if abs(d) < 0.05: arrow = ""
                ...
                arrow = f" <span style='color:#15A46E;font-size:9px'>▲</span>"
            else:
                arrow = f" <span style='color:#DC2626;font-size:9px'>▼</span>"
        except: pass
...
def render_mix_builder(df_active, all_funds_list):
    ...
    st.markdown("<div class='card-title'>⚙️ בניית תמהיל</div>", unsafe_allow_html=True)
    ...
    for key, label, default_on in params:
        ...
        with c1:
            inc[key] = True  # always include in score
            val = st.slider(
                label, 0.0, 100.0,
    ...
    # ── Calc button (outside card, prominent) ──
    ...
    with st.expander("⚙️ הגדרות מתקדמות", expanded=False):
        ...
        # Constraint mode per param (hidden by default, compact)
        ...
        # Manager filter
        all_mgrs = sorted(df_active["manager"].unique().tolist())
        n_total  = len(all_mgrs)
        real_sel_mgrs = st.session_state.get("selected_managers") or all_mgrs
        sel_mgrs = [m for m in real_sel_mgrs if m in all_mgrs and m != "__none__"]
        ...
        with mb1:
            if st.button("בחר הכל", key="mgr_all2", use_container_width=True):
                st.session_state["selected_managers"] = all_mgrs.copy()
                st.rerun()
        with mb2:
            if st.button("נקה הכל", key="mgr_none2", use_container_width=True):
                # Fix 5: properly clear - use empty list sentinel
                st.session_state["selected_managers"] = ["__none__"]
                st.session_state["_mgr_clear_flag"] = True
                st.rerun()

        # Fix 5: check the clear flag to force all checkboxes unchecked
        ...
        for i, mgr in enumerate(all_mgrs):
            with mgr_cols[i % 3]:
                fc = df_active[df_active["manager"] == mgr]["fund"].nunique()
                # When cleared, force value=False regardless of widget state
...
<div class='card' style='text-align:center;padding:30px 18px'>
  ...
  <div style='font-size:13px;color:#6B7280'>הגדר יעדים ולחץ <b>חשב חלופות</b></div>
    ...
    items = best.get("weights_items") or _weights_items(
        best.get("weights"), best.get("קופות",""), best.get("מסלולים",""))
    ...
    bl = baseline or {}
    chips_html = (
        _chip("מניות",   stocks,   "{:.1f}%", False, bl.get("stocks"))   +
        _chip('חו"ל',    foreign,  "{:.1f}%", False, bl.get("foreign"))  +
        _chip('מט"ח',    fx,       "{:.1f}%", False, bl.get("fx"))       +
        _chip("לא-סחיר", illiquid, "{:.1f}%", True,  bl.get("illiquid")) +
        _chip("שארפ",    sharpe,   "{:.2f}",  False, bl.get("sharpe"))   +
        _chip("שירות",   service,  "{:.0f}",  False, bl.get("service"))
    )
    ...
    st.markdown(f"""
<div class='card'>
  <div class='card-title'>🏆 החלופה הטובה ביותר</div>
  <div style='display:flex;align-items:flex-end;gap:10px;margin-bottom:8px'>
    <div>
      <div class='br-score'>{score_pct:.0f}</div>
      <div class='br-score-lbl'>ציון התאמה</div>
    </div>
    <div style='flex:1;font-size:11px;color:#6B7280;padding-bottom:4px'>{_esc(weights_disp)}</div>
  </div>
  <div class='br-managers'>🏢 {_esc(managers)}</div>
  <div class='br-tracks'>📁 {_esc(tracks)}</div>
  {sharpe_note}
  <div class='br-chips'>{chips_html}</div>
</div>
""", unsafe_allow_html=True)
...
def render_results_table(rows, baseline, voting_configured):
    ...
    def tracks_with_weights_html(rrow):
        """Build multi-line cell: 35% מסלול כללי (full name) per fund"""
        ...
        for it in items:
            ...
            if label:
                parts.append(f"<b>{_esc(pct)}%</b>&nbsp;{_esc(label)}")
        return "<br>".join(parts) if parts else "—"

    # Build table HTML – NO separate ▼ column; label cell IS the clickable action
    header_cells = "".join(f"<th>{h}</th>" for h in [
        "לחץ לבחירה ▼", "מסלולים ומשקלים", "מנהלים", "מניות", 'חו"ל', 'מט"ח', "לא-סחיר", "שארפ", "שירות"
    ])
    ...
    for rrow in rows:
        ...
        mgrs_short  = str(rrow.get("מנהלים",""))[:50]
  ...
  <td style='font-size:11px;line-height:1.7'>{tracks_cell}</td>
  <td style='font-size:11px;color:#6B7280'>{_esc(mgrs_short)}</td>
  <td class='num'>{fmt_val(rrow,'מניות (%)')}</td>
  <td class='num'>{fmt_val(rrow,'חו"ל (%)')}</td>
  <td class='num'>{fmt_val(rrow,'מט"ח (%)')}</td>
  <td class='num'>{fmt_val(rrow,'לא\u05d3\u05b9-\u05e1\u05d7\u05d9\u05e8 (%)')}</td>
  {sharpe_cell}
    ...
    if selected_alt:
        ...
        if sel_row:
            items = sel_row.get("weights_items") or _weights_items(
                sel_row.get("weights"), sel_row.get("קופות",""), sel_row.get("מסלולים",""))
            st.markdown(f"""
...
<div style='font-size:13px;font-weight:800;color:#1F3A5F;margin-bottom:8px'>
  📊 {_esc(selected_alt)} — פרטים
            ...
            with d3:
                st.metric("לא-סחיר", _pct(sel_row.get("לא\u05d3\u05b9-\u05e1\u05d7\u05d9\u05e8 (%)","—")))
                st.metric("שירות",   _num(sel_row.get("שירות משוקלל","—"), 0))
            ...
            if baseline:
                ...
                for lbl, bl_key, row_key, lower in [
                    ...
                    ("שארפ","sharpe","שארפ משוקלל",False), ("שירות","service","שירות משוקלל",False),
                ]:
                    try:
    ...
    with st.expander("⚖️ השוואת מסלולי השקעה", expanded=auto_open):
        ...
        for _, row in comp_df.iterrows():
            ...
            for col in comp_df.columns:
                ...
                if col in num_cols:
                    try:
                        dec = 2 if col=="שארפ" else (0 if col=="שירות" else 1)
                        unit = "%" if "%" in col else ""
...
if res and recs:
    ...
    for _key, rrow, title in [
        ...
        r = dict(rrow)
        r["חלופה"]        = title
        r["weights_items"] = _weights_items(r.get("weights"), r.get("קופות",""), r.get("מסלולים",""))
